...
runPowerBetaNullNoise <- function(rho = 0.6, shape = c(1,10),
                           N = 50, 
                           sampleN = 1000, 
  ...
  res <- foreach(i = seq_len(sampleN), 
    ...
    if(runif(1) < propTrue){
      ...
      x <- sample_mat[,1] + rlaplace(nrow(sample_mat), 0, 0.06676164)
      y <- sample_mat[,2]
    } else {
...
load("beta_1_10_dist_power_analysis_withkci_withnoise.RData")
...
pdf("power_beta_1_10_with_pearson_delta_0_13_withkci_withnoise.pdf", height = 6, width=9)
ggplot(toPlot, aes(x=`Effect Size`, y=Power, col=Method)) + geom_line(size=1.5) + ggtitle(paste0("Noised Beta(1,10), N=100,\nDelta = 0.13, alpha = ", alpha)) + pres_ready
dev.off()



load("beta_1_2__4_5_dist_power_analysis_withkci_withnoise.RData")
...
pdf("power_beta_1_2__4_5_with_pearson_delta_0_13_withnoise.pdf", height = 6, width=9)
ggplot(toPlot, aes(x=`Effect Size`, y=Power, col=Method)) + geom_line(size=1.5) + ggtitle(paste0("Noised Beta(1.2, 4.5), N=100,\nDelta = 0.13, alpha = ", alpha)) + pres_ready
dev.off()
